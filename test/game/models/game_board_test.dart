...
void main() {
  ...
  group('ClearResult', () {
    ...
    test('ClearResult equality', () {
      final result1 = ClearResult(clearedRows: [0, 1], clearedCols: [2]);
      final result2 = ClearResult(clearedRows: [0, 1], clearedCols: [2]);
      expect(result1, equals(result2));
    });

    test('ClearResult inequality', () {
      final result1 = ClearResult(clearedRows: [0], clearedCols: []);
      final result2 = ClearResult(clearedRows: [1], clearedCols: []);
  ...
  group('GameBoard', () {
    ...
    group('canPlace', () {
      test('can place single cell at (0,0) on empty board', () {
        final board = GameBoard.empty();
        expect(board.canPlace(singlePiece, GridPoint(0, 0)), isTrue);
      });

      test('can place single cell at (7,7) on empty board (corner)', () {
        final board = GameBoard.empty();
        expect(board.canPlace(singlePiece, GridPoint(7, 7)), isTrue);
      });

      test('cannot place single cell at (-1, 0) (out-of-bounds x)', () {
        final board = GameBoard.empty();
        expect(board.canPlace(singlePiece, GridPoint(-1, 0)), isFalse);
      });

      test('cannot place single cell at (8, 0) (out-of-bounds x)', () {
        final board = GameBoard.empty();
        expect(board.canPlace(singlePiece, GridPoint(8, 0)), isFalse);
      });

      test('cannot place single cell at (0, 8) (out-of-bounds y)', () {
        final board = GameBoard.empty();
        expect(board.canPlace(singlePiece, GridPoint(0, 8)), isFalse);
      });

      test('cannot place at occupied cell', () {
        var board = GameBoard.empty();
        final result1 = board.place(singlePiece, GridPoint(3, 3));
        board = result1.board;

        expect(board.canPlace(singlePiece, GridPoint(3, 3)), isFalse);
      });

      test('cannot place 2-cell horizontal piece at (7,0)', () {
        final board = GameBoard.empty();
        expect(board.canPlace(hPair, GridPoint(7, 0)), isFalse);
      });

      test('can place 2-cell horizontal piece at (6,0)', () {
        final board = GameBoard.empty();
        expect(board.canPlace(hPair, GridPoint(6, 0)), isTrue);
      });
    });
    ...
    group('place', () {
      test('placing single cell at (2,2) makes isOccupied(2,2) true', () {
        final board = GameBoard.empty();
        final result = board.place(singlePiece, GridPoint(2, 2));
      ...
      test('placing at occupied cell throws StateError', () {
        ...
        expect(
          () => board.place(singlePiece, GridPoint(3, 3)),
          throwsStateError,
        );
      });
      ...
      test('row clear: fill row 0, clearedRows contains 0', () {
        var board = GameBoard.empty();
        final result = board.place(hBar, GridPoint(0, 0));
      ...
      test('col clear: fill col 0, clearedCols contains 0', () {
        var board = GameBoard.empty();
        final result = board.place(vBar, GridPoint(0, 0));
      ...
      test(
        ...
        var result = board.place(row3, GridPoint(0, 3));
        board = result.board;
        ...
        result = board.place(col5, GridPoint(5, 0));
        board = result.board;

        // Place the final piece at the intersection
        result = board.place(singlePiece, GridPoint(5, 3));
      ...
      test('original board is not modified by place', () {
        ...
        originalBoard.place(singlePiece, GridPoint(3, 3));
      ...
      test('multiple placements return correct state', () {
        ...
        var result = board.place(singlePiece, GridPoint(0, 0));
        board = result.board;
        expect(board.isOccupied(0, 0), isTrue);

        // Place second piece at (1, 0)
        result = board.place(singlePiece, GridPoint(1, 0));
        board = result.board;
    ...
    group('hasValidMove', () {
      ...
      test('hasValidMove with multiple pieces returns true if any fits', () {
        ...
        final result = board.place(singlePiece, GridPoint(0, 0));
        board = result.board;
    ...
    group('immutability and snapshots', () {
      test('snapshot returns board equal in state', () {
        var board = GameBoard.empty();
        final result1 = board.place(singlePiece, GridPoint(2, 2));
        board = result1.board;
      ...
      test('after placing more pieces on original, snapshot is unchanged', () {
        ...
        final result2 = board.place(singlePiece, GridPoint(3, 3));
        board = result2.board;
      ...
      test('two empty boards are independent', () {
        ...
        final result = board1.place(singlePiece, GridPoint(0, 0));
      ...
      test('snapshot is fully independent copy', () {
        var board = GameBoard.empty();
        final result1 = board.place(singlePiece, GridPoint(0, 0));
        board = result1.board;

        final snapshot = board.snapshot();

        final result2 = board.place(singlePiece, GridPoint(1, 1));
        final result3 = result2.board.place(singlePiece, GridPoint(2, 2));
    ...
    group('edge cases and complex scenarios', () {
      test('simultaneous multi-row clear when one piece completes two rows', () {
        ...
        var result = board.place(partialRow, GridPoint(0, 0)); // fills row 0 except x=4
        board = result.board;
        result = board.place(partialRow, GridPoint(0, 1)); // fills row 1 except x=4
        board = result.board;
        ...
        result = board.place(vPair, GridPoint(4, 0));
      ...
      test('simultaneous multi-col clear when one piece completes two cols', () {
        ...
        var result = board.place(partialCol, GridPoint(0, 0)); // fills col 0 except row 4
        board = result.board;
        result = board.place(partialCol, GridPoint(1, 0)); // fills col 1 except row 4
        board = result.board;
        ...
        result = board.place(hPairPiece, GridPoint(0, 4));
      ...
      test('placing piece does not clear non-full lines', () {
        ...
        var result = board.place(partialRow, GridPoint(0, 0));
        board = result.board;
    ...
    group('equality and hashing', () {
      ...
      test('board with piece at (2,2) is not equal to empty board', () {
        ...
        final result = board1.place(singlePiece, GridPoint(2, 2));
        board1 = result.board;
